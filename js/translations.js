const translations = {
   bg: {
      // Navigation
      'nav.about': 'За клуба',
      'nav.judo': 'Джудо',
      'nav.kids': 'За деца',
      'nav.enthusiasts': 'За любители',
      'nav.competitors': 'За състезатели и напреднали',
      'nav.schedule': 'График',
      'nav.contacts': 'Контакти',

      // Mobile navigation
      'mobile.about': 'За клуба',
      'mobile.kids': 'Джудо за деца',
      'mobile.enthusiasts': 'Джудо за любители',
      'mobile.competitors': 'Джудо за състезатели',
      'mobile.schedule': 'График',
      'mobile.contacts': 'Контакти',

      // About the club
      'about.title': 'За клуба',
      'about.p1':
         'Джудо Център Кано е първата детска школа в град Варна, основана през 2010 година от Сенсей Дехгхани - дипломиран в КОДОКАН, Япония с над 45 години опит състезателна и треньорска дейност.',
      'about.p2':
         'Школата има дългогодишна история и много практика в работата с деца, възрастни и хора в неравностойно положение, като през годините е създала много шампиони в местни, и международни турнири. Клубът е лицензиран от Министерството на младежта и спорта, и Българската федерация по джудо.',
      'about.p3':
         'Джудо Център Кано разполага с нова база в модерна сграда Клас А. Занятията се провеждат в добре оборудвана зала със собствена климатизация на професионална джудо настилка - татами.',

      // Judo for kids
      'kids.title': 'Джудо за деца',
      'kids.p1':
         'Заради своята същност, японска философия и дисциплина, и като единственото бойно изкуство без удари, джудо не е само спорт. Джудо е начин на живот, който оформя личности и занятията за деца стават все по-търсени, и предпочитани от родителите по цял свят.',
      'kids.p2': 'Джудо учи на дисциплина, уважение и постоянство!',
      'kids.p3':
         'Гордеем се, че имаме опита от практиката да обучаваме деца в различни възрастови групи след навършване на 5 годишна възраст. Графикът на занятията може да видите тук:',

      // Judo for enthusiasts
      'enthusiasts.title': 'Джудо за любители',
      'enthusiasts.p1':
         'В нашия спорт няма ограничения за пол и възраст, което го прави уникален, търсен и уважаван от милиони хора по света. Всеки, който желае да практикува джудо за лично усъвършенстване и придобиване на знания да се чувства поканен.',

      // Judo for competitors
      'competitors.title': 'Джудо за състезатели и напреднали',
      'competitors.p1':
         'Джудо Център Кано предлага обучение, подготовка и сертифициране на състезатели и напреднали, съобразено с правилата на Международната джудо федерация – IJF и програми на Кодокан, Япония.',
      'competitors.p2':
         'Всеки желаещ да продължи обучението си е добре дошъл да ни посети на място в някой от работните дни на Центъра.',

      // Schedule
      'schedule.title': 'График',
      'schedule.monday': 'Понеделник',
      'schedule.tuesday': 'Вторник',
      'schedule.wednesday': 'Сряда',
      'schedule.thursday': 'Четвъртък',
      'schedule.friday': 'Петък',
      'schedule.saturday': 'Събота',
      'schedule.sunday': 'Неделя',
      'schedule.forKids': 'За деца',
      'schedule.forAdvanced': 'За напреднали и любители',
      'schedule.dayOff': 'Почивен ден',

      // Contacts
      'contacts.title': 'Контакти',
      'contacts.address': 'АДРЕС',
      'contacts.phone': 'ТЕЛЕФОН',
      'contacts.addressText': 'бул. Ян Хунияди №31',

      // Buttons
      'btn.schedule': 'ГРАФИК НА ЗАНЯТИЯТА',

      // Footer
      'footer.copyright': '©Copyright, 2025 - Judo Center Kano',
   },
   en: {
      // Navigation
      'nav.about': 'About the Club',
      'nav.judo': 'Judo',
      'nav.kids': 'For Children',
      'nav.enthusiasts': 'For Enthusiasts',
      'nav.competitors': 'For Competitors and Advanced',
      'nav.schedule': 'Schedule',
      'nav.contacts': 'Contacts',

      // Mobile navigation
      'mobile.about': 'About the Club',
      'mobile.kids': 'Judo for Children',
      'mobile.enthusiasts': 'Judo for Enthusiasts',
      'mobile.competitors': 'Judo for Competitors',
      'mobile.schedule': 'Schedule',
      'mobile.contacts': 'Contacts',

      // About the club
      'about.title': 'About the Club',
      'about.p1':
         'Judo Center Kano is the first children\'s school in the city of Varna, founded in 2010 by Sensei Dehghani - a graduate of KODOKAN, Japan with over 45 years of competitive and coaching experience.',
      'about.p2':
         'The school has a long history and extensive experience in working with children, adults and people with disabilities, and over the years has produced many champions in local and international tournaments. The club is licensed by the Ministry of Youth and Sports, and the Bulgarian Judo Federation.',
      'about.p3':
         'Judo Center Kano has a new facility in a modern Class A building. Classes are held in a well-equipped hall with its own air conditioning on a professional judo mat - tatami.',

      // Judo for kids
      'kids.title': 'Judo for Children',
      'kids.p1':
         'Due to its nature, Japanese philosophy and discipline, and as the only martial art without strikes, judo is not just a sport. Judo is a way of life that shapes personalities, and classes for children are becoming increasingly sought after and preferred by parents around the world.',
      'kids.p2': 'Judo teaches discipline, respect and perseverance!',
      'kids.p3':
         'We are proud that we have the experience and practice to train children in different age groups from the age of 5. You can see the class schedule here:',

      // Judo for enthusiasts
      'enthusiasts.title': 'Judo for Enthusiasts',
      'enthusiasts.p1':
         'In our sport, there are no restrictions on gender and age, which makes it unique, sought after and respected by millions of people around the world. Anyone who wishes to practice judo for personal improvement and knowledge acquisition is welcome.',

      // Judo for competitors
      'competitors.title': 'Judo for Competitors and Advanced',
      'competitors.p1':
         'Judo Center Kano offers training, preparation and certification of competitors and advanced practitioners, in accordance with the rules of the International Judo Federation - IJF and programs of Kodokan, Japan.',
      'competitors.p2':
         'Anyone wishing to continue their training is welcome to visit us in person on any of the Center\'s working days.',

      // Schedule
      'schedule.title': 'Schedule',
      'schedule.monday': 'Monday',
      'schedule.tuesday': 'Tuesday',
      'schedule.wednesday': 'Wednesday',
      'schedule.thursday': 'Thursday',
      'schedule.friday': 'Friday',
      'schedule.saturday': 'Saturday',
      'schedule.sunday': 'Sunday',
      'schedule.forKids': 'For Children',
      'schedule.forAdvanced': 'For Advanced and Enthusiasts',
      'schedule.dayOff': 'Day Off',

      // Contacts
      'contacts.title': 'Contacts',
      'contacts.address': 'ADDRESS',
      'contacts.phone': 'PHONE',
      'contacts.addressText': '31 Yan Hunyadi Blvd.',

      // Buttons
      'btn.schedule': 'CLASS SCHEDULE',

      // Footer
      'footer.copyright': '©Copyright, 2025 - Judo Center Kano',
   },
};
